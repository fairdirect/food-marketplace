...
class Epelia_Helper{
	...
	public static function make_url($str,$divider="-"){
		$ar_replaces 	= array(
							array("ä","ae"),
							array("ö","oe"),
							array("ü","ue"),
		...
		foreach($ar_replaces as $r)
			$str	= str_replace($r[0], $r[1], $str);

		$str		= self::remove_accents($str);
		$str		= preg_replace("/[^0-9a-zA-Z_".$divider."]/","",$str);
		if($divider!=""){
			while(substr_count($str,$divider.$divider)>0)
				$str= str_replace($divider.$divider,$divider,$str);
		}
		return $str;
	}
