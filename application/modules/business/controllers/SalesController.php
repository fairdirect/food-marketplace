...
class Business_SalesController extends Zend_Controller_Action
{
    ...
    private function getHeader($pdf, $order){
        ...
        $pdf->Ln();
        $pdf->setX(10);
        $pdf->MultiCell(180, 5, utf8_decode('Epelia und ' . $order->getShop()->name . ' begrüßen Sie als Kunden auf unserem Marktplatz für Lebensmittel. Wir bedanken uns herzlich für Ihre Bestellung.'));
        $pdf->Ln();
    ...
    private function getBody($pdf, $order){
        ...
        $pdf->Ln();
        $pdf->Ln();
        $pdf->Cell(180, 5, utf8_decode('Zahlungsweise: Die Abwicklung der Zahlung erfolgte über den Epelia Treuhandservice.'));
        if($order->getShop()->small_business){
            $pdf->Ln();
