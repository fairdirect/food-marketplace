...
class Model_User extends Model_ModelAbstract
{
    ...
    public static function getAll($limit = null, $offset = null, $search = ''){
        ...
        if($search){
            $search = '%' . $search . '%';
            $query .= ' WHERE u.email ILIKE ' . $db->quote($search) . ' OR u.id = ' . $db->quote(intVal(str_replace('%', '', $search))) . ' OR a.firstname ILIKE ' . $db->quote($search) . ' OR a.name ILIKE ' .$db->quote($search) . " OR (a.firstname || ' ' || a.name) ILIKE " . $db->quote($search) . ' OR a.street ILIKE ' . $db->quote($search) . ' OR a.zip ILIKE ' .$db->quote($search) . ' OR a.city ILIKE ' . $db->quote($search);
        }
