...
try{
    //$postgres = new PDO('pgsql:dbname=epelia_testing;host=localhost', 'epelia_testing', 'Hag6-Ja2h(gDamHzt');
...
//$mysql = new mysqli('localhost', 'shoesel', 'hZgst5$cfsWp=.', 'epelia');
$mysql = new mysqli('localhost', 'root', '', 'epelia');
                        ...
                        while($shipping = $shipping_res->fetch_object()){
                            try{
                                $ret = $shipping_query->execute(array(
                                    ...
                                    ($shipping->forwarding_charges_free > 0) ? $shipping->forwarding_charges : null // free_from
                                ));
